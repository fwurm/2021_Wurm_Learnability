function output = model_gambLearn_freeLR_partial(x,data)

% Reinforcement learning model with variable learning rates and partial updating
%
...
%catch parameters
lr_learn = x(1); %learning rate learning
lr_gamb = x(2); %learning rate gambling
...
for iB = 1:data.nB
    ...
    for iT = 1:data.nT
        ...
        if c ~= 3
            ...
            % update values
            if y == 0
                v(s,c) = v(s,c) + lr_learn*rpe; %chose option
%                 v(s,3-c) = v(s,3-c) - lr_learn*rpe;
            elseif y == 1
                v(s,c) = v(s,c) + lr_gamb*rpe; %chose option
...
%         possib(union(c,3)) = [];
%         if y == 0
%             v(s,possib) = (1-lr_learn).*v(s,possib);
%         elseif y == 1
%             v(s,possib) = (1-lr_gamb).*v(s,possib);
%         else
%             error('no yoking condition detected')
%         end
